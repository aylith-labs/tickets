...
function isolatedEnv(profile, gitPath) {
	...
	return {
		...
		NPM_CONFIG_GLOBALCONFIG: join(profile, 'npm-globalrc'),
...
async function download(url, file) {
	...
	assert.equal(response.status, 200, url);
	assert(
		['github.com', 'release-assets.githubusercontent.com', 'registry.npmjs.org'].includes(
...
async function start(state, port, label) {
	...
	const job = {
		...
		stdout: '',
		stderr: '',
		record: { label, pid: child.pid, port, startedAt: now(), executable: state.binary },
	};
	live.add(job);
...
try {
	if (phase === 'prepare') {
		...
		const release = await getJson(
			'https://api.github.com/repos/aylith-labs/tickets/releases/tags/v0.1.3',
			'github-release.json',
		);
		assert.equal(release.tag_name, 'v0.1.3');
		assert.equal(release.draft, false);
		const ref = await getJson(
			'https://api.github.com/repos/aylith-labs/tickets/git/ref/tags/v0.1.3',
			'github-tag.json',
		);
		...
		await check('official-windows-x64-asset', {
		...
		for (const name of ['tickets', 'tickets-server', 'tickets-core', 'tickets-ui', 'tickets-tui']) {
			...
			await check(`${name}-tarball`, {
				bytes: packageBytes.length,
				sha256: sha(packageBytes),
				integrity: metadata.dist.integrity,
				files: files.length,
				browserAssets: files.filter((file) => /\.(html|css)$|(^|\/)components\.js$|apps\/web\/dist/.test(file)),
				engines: packageJson.engines,
				scripts: packageJson.scripts,
			});
		}
		receipt.findings.push(
			'The five npm archives do not bundle the browser app; npm runtime is not installed or tested.',
		);
		...
		for (const kind of ['default', 'folder']) {
			const profile = join(root, `${kind}-profile`),
				repoDir = join(root, `${kind}-repo`);
			...
			const homeProof = await run(
				bun,
				[
					'-e',
					'console.log(JSON.stringify({home:require("node:os").homedir(),bun:Bun.version,platform:process.platform,arch:process.arch}))',
				],
				repoDir,
				env,
				`${kind}-home-proof`,
			...
			for (const [key, value] of [
				['user.name', 'Tickets first-use fixture'],
				['user.email', 'tickets-proof@localhost'],
			]) {
			...
			assert.equal(
				(
					await run(
						git,
						['commit', '--allow-empty', '-m', 'Initialize isolated first-use fixture'],
						repoDir,
						env,
						`${kind}-initial-commit`,
					)
			...
			if (init.code !== 0)
				receipt.findings.push(`${kind} init failed; inspect its command stderr before claiming first-use acceptance.`);
		...
		page.on('response', (response) =>
			receipt.responses.push({ at: now(), status: response.status(), url: response.url() }),
		);
		...
		const responseFor = (method) =>
			page.waitForResponse(
				(response) =>
					response.request().method() === method && new URL(response.url()).pathname.startsWith('/api/tickets'),
			);
		try {
			...
			await page.getByText('No tickets yet — capture the first one above.', { exact: true }).waitFor();
			await check('fresh-browser-empty-project', {
				base,
				project: state.projectName,
				browser: browserRecord.version,
				playwrightVersion,
			});
			...
			await check('browser-edited-and-status-saved', {
				id: created.id,
				title: editedTitle,
			...
			try {
				await fetch(`${base}/api/projects`, { signal: AbortSignal.timeout(1500) });
			} catch {
				unavailable = true;
			}
			assert(unavailable, 'The old daemon must actually be stopped');
			await check('daemon-stopped-confirmed', {
				pid: job.record.pid,
				exitCode: job.record.exitCode,
				signal: job.record.signal,
				port,
			});
			...
			await check('process-restart-and-browser-reload-persist', {
				id: created.id,
				oldPid: receipt.jobs[0].pid,
				newPid: job.record.pid,
				fullTicketEqual: true,
				tickets: 1,
			});
			...
			const layout = await page.evaluate(() => {
					...
					for (const element of rootNode.querySelectorAll('*')) {
						...
						if (element.matches('input,select,textarea,button,a')) {
							...
							if (rect.width && rect.height)
								controls.push({
									tag: element.tagName,
									name: element.getAttribute('name') || element.textContent?.trim().slice(0, 60),
									x: rect.x,
									right: rect.right,
									width: rect.width,
								});
				...
				return {
					...
					overflowControls: controls.filter((item) => item.x < 0 || item.right > innerWidth),
				};
			});
			await check('narrow-layout-observation', layout);
			if (layout.documentWidth > layout.viewport || layout.overflowControls.length)
				receipt.findings.push('Narrow layout overflows the viewport.');
			...
			assert(
				receipt.requests.every(
					(request) => !/\/(launch|enrich|attachments|prompt|restore)(\/|$)/.test(new URL(request.url).pathname),
				),
			);
			await check('bounded-network-and-page-errors', {
